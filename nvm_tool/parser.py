...
class NvMConfigParser:
        ...
        if containers_element is not None:
            for container in containers_element:
                ...
                if (definition_ref.text or "").strip() != NVM_BLOCK_CONTAINER_DEFINITION_REF:
                    continue
                blocks.append(self._parse_block_container(container))

        self._validate_unique_ids(blocks, "previous ARXML")
        self._validate_unique_short_names(blocks, "previous ARXML")
        self._warn_reserved_ids(blocks, str(path))
    ...
    def _validate_unique_ids(self, blocks: List[NvMBlock], source_label: str) -> None:
        ...
        for block in blocks:
            ...
            if existing is not None:
                raise ValueError(
                    f"Duplicate block_id {block.block_id} found in {source_label} for "
    ...
    def _validate_unique_generated_names(self, blocks: List[NvMBlock], source_label: str) -> None:
        ...
        self._validate_unique_short_names(blocks, source_label)

    def _validate_unique_short_names(self, blocks: List[NvMBlock], source_label: str) -> None:
        self._validate_unique_strings(
            blocks,
